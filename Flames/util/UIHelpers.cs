...
namespace Flames.UI 
{
    ...
    {        
        ...
        public static void HandleCommand(string text) {
            ...
            if (cmd == null) { 
                Logger.Log(LogType.CommandUsage, "(Flames): Unknown command \"{0}\"", name); return; 
            }
            if (!cmd.SuperUseable) { 
                Logger.Log(LogType.CommandUsage, "(Flames): /{0} can only be used in-game.", cmd.name); return; 
            }

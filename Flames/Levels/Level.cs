...
namespace Flames 
{
    ...
    {        
        ...
        public static void LoadMetadata(Level lvl) {
            try {
                ...
                if (lvl.Config.Load(propsPath)) {
                    ...
                    Logger.Log(LogType.FlameMessage, ".properties file for level {0} was not found.", lvl.MapName);
                }
            } catch (Exception e) {
                Logger.LogError(e);
            }
        ...
        public void UpdateBlockHandlers(BlockID block) {
            bool nonSolid = !Flames.Blocks.CollideType.IsSolid(CollideType(block));
